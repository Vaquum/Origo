...
from origo.sources.adapters import binance_spot_agg_daily
...
from origo.sources.profiles import spot_agg_consumers
from origo.sources.profiles.formulas.spot_agg_series import SPECS as SPOT_AGG_SERIES_SPECS
from origo.sources.storage import SourceStore

from .acceptance_cases import SPOT_AGG_CASE
from .test_binance_spot_agg_daily_source_adapter import archive_response

DAY = '2017-08-17'
ASSET = 'build_binance_spot_aggtrades_canonical_revision_origo'
...
def ready_job(
    ...
    monkeypatch.setattr(binance_spot_agg_daily, 'get_response', archive_response)
    ...
    FakeHfApi.calls = []
    monkeypatch.setattr(spot_agg_consumers, 'HfApi', FakeHfApi)
    monkeypatch.setattr(
    ...
    spec = replace(
        ...
        orchestration=replace(BINANCE_SPOT_AGGTRADES_SPEC.orchestration, retry_count=0),
    )
...
def test_one_job_prepares_verifies_and_publishes_all_spot_agg_files(
    ...
    # test_spot_agg_huggingface_upload_records_every_rendered_series.
...
def test_spot_agg_file_failure_fails_job_and_retry_keeps_verified_generation(
...
def test_spot_agg_unavailable_day_blocks_publication_and_preserves_completed_day(
    ready_job: tuple[SourceStore, DagsterInstance, SourceBundle], tmp_path: Path
) -> None:
    store, instance, bundle = ready_job
    job = Definitions(
    ...
    assert statuses['2017-08-19'].value == 'FAILED'
    for sensor in bundle.sensors:
        if any(
...
def test_spot_agg_new_verified_data_automatically_requests_every_canonical_only_consumer(
    ...
    assert canonical.execute_in_process(instance=instance, partition_key='2017-08-18').success
    assert store.canonical_ready()
    ...
    for consumer in canonical_only:
        ...
        with build_sensor_context(instance=instance, definitions=definitions) as context:
            assert sensor.evaluate_tick(context).run_requests == []
def test_spot_agg_publication_follows_canonical_state_across_provisional_refreshes(
    ready_job: tuple[SourceStore, DagsterInstance, SourceBundle], tmp_path: Path
) -> None:
    ...
    assert canonical_job.execute_in_process(instance=instance, partition_key='2017-08-18').success
    ...
    assert pinned['pinned_token'] == store.snapshot().token
def test_spot_agg_canonical_day_retires_failed_provisional_intervals_inside_it(
    ready_job: tuple[SourceStore, DagsterInstance, SourceBundle], tmp_path: Path
) -> None:
    from uuid import uuid4
...
def test_spot_agg_native_backfill_failure_holds_publication_after_last_range_succeeds(
    ...
    with DagsterInstance.local_temp(str(tmp_path)) as instance:
        ...
        for day, run_status in (
            ('2017-08-19', DagsterRunStatus.FAILURE),
            ('2017-08-18', DagsterRunStatus.SUCCESS),
        ):
...
def test_spot_agg_native_job_backfill_waits_for_own_selected_generations(
    ready_job: tuple[SourceStore, DagsterInstance, SourceBundle], tmp_path: Path
) -> None:
    from dagster._core.execution.backfill import BulkActionStatus, PartitionBackfill
    from dagster._core.storage.tags import BACKFILL_ID_TAG
    ...
    assert job.execute_in_process(instance=instance, partition_key='2017-08-18').success
    ...
    parent = PartitionBackfill(
        ...
        partition_names=[DAY, '2017-08-18'],
    )
    ...
    assert instance.get_materialized_partitions(AssetKey(ASSET)) == {DAY, '2017-08-18'}
...
def test_spot_agg_huggingface_upload_records_every_rendered_series(
    ready_job: tuple[SourceStore, DagsterInstance, SourceBundle],
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    from origo.sources.profiles import spot_agg_consumers
    from origo.sources.profiles.formulas import spot_agg_huggingface as agg_snapshot
    ...
    monkeypatch.setattr(agg_snapshot, 'get_spot_agg_klines_from_1m_projection', lambda **kwargs: frame)
    monkeypatch.setattr(
...
def test_spot_agg_mount_renders_post_cutoff_month_and_sweeps_only_own_staging(
    ready_job: tuple[SourceStore, DagsterInstance, SourceBundle],
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    from origo.sources.lifecycle import SourceRuntime
    from origo.sources.profiles.formulas.spot_agg_series import SPECS
    ...
    own = parquet / '.binance_spot_aggtrades-staging-stale'
    ...
    assert not list(parquet.glob('.binance_spot_aggtrades-staging-*'))
...
def test_source_inventory_is_explicit() -> None:
    assert SPOT_AGG_CASE.inventory == (
        'spot_agg_time_1m',
        'spot_agg_time_15m',
        'spot_agg_time_30m',
        'spot_agg_time_1h',
    ...
    assert tuple(series.name for series in SPOT_AGG_SERIES_SPECS) == SPOT_AGG_CASE.inventory
