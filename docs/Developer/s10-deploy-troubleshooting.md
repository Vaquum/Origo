...
- `ERROR: ORIGO_AUDIT_LOG_RETENTION_DAYS must be present and non-empty in .env.example.`
  - fix: ensure root `.env.example` contains valid `ORIGO_AUDIT_LOG_RETENTION_DAYS` (`>=365`) and rerun deploy so workflow syncs it into `/opt/origo/deploy/.env`.
- `ERROR: ORIGO_AUDIT_LOG_RETENTION_DAYS must be set and non-empty.`
  - fix: ensure root `.env.example` contains valid `ORIGO_AUDIT_LOG_RETENTION_DAYS` (`>=365`) and rerun deploy so workflow syncs it into `/opt/origo/deploy/.env`.
