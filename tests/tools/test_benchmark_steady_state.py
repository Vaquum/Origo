...
def test_missing_real_input_cache_refuses_before_runtime_creation(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    clean_environment(monkeypatch)
    cache = tmp_path / 'missing-cache'
    monkeypatch.setattr(trial, 'CACHE', cache)
    # Keep the committed authentic provenance, but explicitly supply an absent
    # local cache. CI must not depend on files existing only on an author's Mac.
    document = json.loads((ROOT / 'tests/fixtures/steady_state/archive_corpus.json').read_text())
    for entry in document['archives']:
        entry['cache_path'] = str(cache / entry['source_key'] / Path(entry['cache_path']).name)
    manifest = tmp_path / 'manifest.json'
    manifest.write_text(json.dumps(document))
    with pytest.raises(PermissionError, match='existing authentic files'):
        trial.validate_request(manifest, tmp_path / 'new', 21600)
