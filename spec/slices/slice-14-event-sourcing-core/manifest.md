...
- Completed pilot cutover for Binance `binance_spot_trades` through canonical events with parity proofs for `native` and `aligned_1s`.
...
- Pilot serving parity for Binance `binance_spot_trades` is proven between legacy baseline and projection-driven path.
...
- Event-sourcing migration is only piloted for Binance `binance_spot_trades` in this slice; full Binance port remains Slice 15 scope.
