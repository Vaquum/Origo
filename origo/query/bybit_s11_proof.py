...
SLICE_DIR = Path('spec/slices/slice-11-bybit-spot-trades-aligned')
INGEST_RESULTS_PATH = SLICE_DIR / 'ingest-results.json'

DAY_WINDOWS: dict[str, tuple[str, str]] = {
    '2024-01-01': ('2024-01-01T00:00:00Z', '2024-01-02T00:00:00Z'),
    '2024-01-02': ('2024-01-02T00:00:00Z', '2024-01-03T00:00:00Z'),
}
...
def _load_ingest_results() -> list[IngestResult]:
    ...
    for idx, item in enumerate(raw_results):
        ...
        gzip_sha256 = _expect_str(
            item_obj.get('gzip_sha256'),
            label=f'ingest_results[{idx}].gzip_sha256',
        )
        csv_sha256 = _expect_str(
            item_obj.get('csv_sha256'),
            label=f'ingest_results[{idx}].csv_sha256',
        )
        source_etag = _expect_str(
            item_obj.get('source_etag'),
            label=f'ingest_results[{idx}].source_etag',
        )
        ...
        canonical_rows.append(
            {
                ...
                'trd_match_id': cast(str, row['trd_match_id']),
            }
        )
...
def _build_acceptance_proof() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    native_envelope = query_native_wide_rows_envelope(
        dataset='bybit_spot_trades',
        select_cols=['trade_id', 'timestamp', 'price', 'size', 'side', 'trd_match_id'],
        time_range=(start_iso, end_iso),
        include_datetime_col=True,
        auth_token=None,
    )
    if (
        native_envelope['mode'] != 'native'
        or native_envelope['source'] != 'bybit_spot_trades'
    ):
        raise RuntimeError('S11-P1 native envelope mode/source mismatch')
    if native_envelope['row_count'] <= 0:
        raise RuntimeError('S11-P1 native envelope row_count must be > 0')
    ...
    if (
        aligned_envelope['mode'] != 'aligned_1s'
        or aligned_envelope['source'] != 'bybit_spot_trades'
    ):
        raise RuntimeError('S11-P2 aligned envelope mode/source mismatch')
    if aligned_envelope['row_count'] <= 0:
        raise RuntimeError('S11-P2 aligned envelope row_count must be > 0')

    aligned_rows = cast(list[dict[str, Any]], aligned_envelope['rows'])
    if len(aligned_rows) == 0:
        raise RuntimeError('S11-P2 aligned rows must be non-empty')
    trade_count_total = 0
    for idx, row in enumerate(aligned_rows):
        trade_count = row.get('trade_count')
        if not isinstance(trade_count, int) or trade_count <= 0:
            raise RuntimeError(
                f'S11-P2 aligned trade_count must be positive int at row={idx}'
            )
        trade_count_total += trade_count

    acceptance_native = {
        'generated_at_utc': datetime.now(UTC).isoformat(),
        'proof_scope': 'Slice 11 P1 native acceptance',
        'dataset': 'bybit_spot_trades',
        ...
        'rows_hash_sha256': _canonical_hash(
            cast(list[dict[str, Any]], native_envelope['rows'])
        ),
    }

    acceptance_aligned = {
        'generated_at_utc': datetime.now(UTC).isoformat(),
        'proof_scope': 'Slice 11 P2 aligned acceptance',
        'dataset': 'bybit_spot_trades',
        'window': {'day': day, 'start_utc': start_iso, 'end_utc': end_iso},
...
def main() -> None:
    ...
    determinism_payload = {
        'generated_at_utc': datetime.now(UTC).isoformat(),
        'proof_scope': 'Slice 11 P3 native + aligned determinism replay',
        'run_1': run_1,
        'run_2': run_2,
    ...
    source_checksum_payload = {
        'generated_at_utc': datetime.now(UTC).isoformat(),
        'proof_scope': 'Slice 11 P4 source checksum and row stats',
    ...
    (SLICE_DIR / 'proof-s11-p1-acceptance.json').write_text(
        json.dumps(acceptance_native, indent=2, sort_keys=True),
        encoding='utf-8',
    )
    (SLICE_DIR / 'proof-s11-p2-aligned-acceptance.json').write_text(
        json.dumps(acceptance_aligned, indent=2, sort_keys=True),
        encoding='utf-8',
    )
    (SLICE_DIR / 'proof-s11-p3-determinism.json').write_text(
        json.dumps(determinism_payload, indent=2, sort_keys=True),
        encoding='utf-8',
    )
    (SLICE_DIR / 'proof-s11-p4-source-checksums.json').write_text(
