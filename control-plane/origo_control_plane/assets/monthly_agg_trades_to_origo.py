...
from origo_control_plane.utils.binance_canonical_event_ingest import (
    parse_binance_spot_agg_trade_csv,
    write_binance_spot_agg_trades_to_canonical,
)
...
@asset(
    ...
    description='Downloads, validates, and writes Binance BTC spot aggregate trades into canonical event log',
)
def insert_monthly_binance_agg_trades_to_origo(
    context: AssetExecutionContext,
) -> dict[str, Any]:
    partition_date_str = context.asset_partition_key_for_output()
    date_parts = partition_date_str.split('-')
    year, month = date_parts[0], date_parts[1]
    month_file_name = f'BTCUSDT-aggTrades-{year}-{month}.zip'
    context.log.info(
...
def _process_month(
    ...
    events = parse_binance_spot_agg_trade_csv(csv_content)
    ...
    try:
        ...
        write_summary = write_binance_spot_agg_trades_to_canonical(
            client=client,
            database=CLICKHOUSE_DATABASE,
            events=events,
        ...
        native_projection_summary = project_binance_spot_agg_trades_native(
            client=client,
            database=CLICKHOUSE_DATABASE,
        ...
        aligned_projection_summary = project_binance_spot_agg_trades_aligned(
            client=client,
            database=CLICKHOUSE_DATABASE,
            partition_ids=partition_ids,
            run_id=context.run_id,
