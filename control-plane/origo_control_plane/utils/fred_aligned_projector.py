...
_PROJECTOR_ID = 'fred_series_metrics_aligned_1s_v1'
...
def _require_source_event_time(event: ProjectorEvent) -> datetime:
    if event.source_event_time_utc is None:
        raise RuntimeError(
            'FRED aligned projector requires source_event_time_utc for every event'
        )
    return _require_utc(
...
class FREDAlignedProjectorSummary:
...
def _aggregate_summaries(
    summaries: list[FREDAlignedProjectorSummary],
) -> FREDAlignedProjectorSummary:
    return FREDAlignedProjectorSummary(
...
def project_fred_series_metrics_aligned(
    *,
    client: ClickhouseClient,
    database: str,
