CREATE TABLE IF NOT EXISTS {{DATABASE}}.bybit_spot_trades (
    symbol LowCardinality(String) CODEC(ZSTD(3)),
    trade_id UInt64 CODEC(Delta(8), ZSTD(3)),
    trd_match_id String CODEC(ZSTD(3)),
    ...
    tick_direction LowCardinality(String) CODEC(ZSTD(3)),
    gross_value Float64 CODEC(ZSTD(3)),
    home_notional Float64 CODEC(ZSTD(3)),
    foreign_notional Float64 CODEC(ZSTD(3))
)
ENGINE = MergeTree()
