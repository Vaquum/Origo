...
def test_perp_canonical_polls_availability_within_five_minutes() -> None:
    # SS-04 observes availability on a bounded cadence, not a guessed daily
    # publish hour. Latest-day 404 skipping remains separately exercised below.
    ...
    for spec in (BINANCE_PERP_TRADES_SPEC, BINANCE_PERP_AGGTRADES_SPEC):
        assert spec.orchestration.canonical_cron == '*/5 * * * *', spec.key


def test_spot_canonical_polls_availability_within_five_minutes() -> None:
    # All four sources share the SS-04 availability observation bound.
