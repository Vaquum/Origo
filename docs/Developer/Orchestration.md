...
stops with a named phase after `--deadline-seconds` (900 in deployment), the compose
start waits at most 600 seconds, the deploy job at most 45 minutes, and a failed
start prints the last 200 daemon log lines. A recovery error before the recording
run exists appears in that log excerpt, not as a Dagster run.
