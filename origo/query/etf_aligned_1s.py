...
_ETF_ALIGNED_OBS_COLUMNS: tuple[str, ...] = (
    'aligned_at_utc',
    'source_id',
    'metric_name',
    'metric_unit',
...
ETFAlignedDataset = Literal['etf_daily_metrics']


@dataclass(frozen=True)
class ETFAligned1sMaterialization:
    table_name: str
    datetime_column: str
    ingest_column: str
...
_ETF_ALIGNED_1S_MATERIALIZATIONS: dict[
    ETFAlignedDataset, ETFAligned1sMaterialization
] = {
    'etf_daily_metrics': ETFAligned1sMaterialization(
...
def _build_grouped_query(
    *,
    database: str,
    materialization: ETFAligned1sMaterialization,
    where_clause: str,
) -> str:
...
def build_etf_aligned_1s_sql(
    *,
    dataset: ETFAlignedDataset,
    window: QueryWindow,
    database: str,
) -> str:
    ...
    materialization = _ETF_ALIGNED_1S_MATERIALIZATIONS[dataset]
...
def _query_etf_aligned_prior_state(
    *,
    dataset: ETFAlignedDataset,
    start_iso: str,
    auth_token: str | None,
    ...
    if isinstance(raw_frame, pl.Series):
        raise RuntimeError(
            'Expected DataFrame from ETF aligned prior-state query, got Series'
        )
    return _shape_aligned_frame(frame=raw_frame, datetime_iso_output=False)


def _build_etf_forward_fill_intervals(
    *,
    ...
    for column in _ETF_ALIGNED_OBS_COLUMNS:
    ...
    normalized = (
        observations.select(_ETF_ALIGNED_OBS_COLUMNS)
...
def query_etf_aligned_1s_data(
    *,
    ...
    raw_frame = pl.DataFrame(arrow_table)
    if isinstance(raw_frame, pl.Series):
        raise RuntimeError('Expected DataFrame from ETF aligned query, got Series')
...
def query_etf_forward_fill_intervals(
    *,
    dataset: ETFAlignedDataset,
    ...
    in_window = query_etf_aligned_1s_data(
        dataset=dataset,
        window=window,
    ...
    prior_state = _query_etf_aligned_prior_state(
        dataset=dataset,
    ...
    started_at = time.time()
    intervals = _build_etf_forward_fill_intervals(
