...
_PROJECTOR_ID = 'fred_series_metrics_native_v1'
_TARGET_TABLE = 'canonical_fred_series_metrics_native_v1'
_INSERT_COLUMNS = (
...
def _decode_metric_value(payload: dict[str, Any]) -> tuple[str | None, int | None, float | None, int | None]:
    ...
    if metric_value_kind == 'bool':
        if metric_value_bool is None or metric_value_text not in {'true', 'false'}:
            raise RuntimeError(
                'Boolean FRED payload requires metric_value_bool and metric_value_text=true|false'
            )
        return (metric_value_text, None, None, metric_value_bool)
    if metric_value_kind == 'int':
        if metric_value_text is None:
            raise RuntimeError('Integer FRED payload requires metric_value_text')
        try:
            value_int = int(metric_value_text)
        except ValueError as exc:
            raise RuntimeError('Integer FRED payload metric_value_text must parse as int') from exc
        return (metric_value_text, value_int, float(value_int), None)
    if metric_value_kind == 'float':
        if metric_value_text is None:
            raise RuntimeError('Float FRED payload requires metric_value_text')
        try:
            value_float = float(Decimal(metric_value_text))
        except InvalidOperation as exc:
            raise RuntimeError('Float FRED payload metric_value_text must parse as decimal') from exc
        return (metric_value_text, None, value_float, None)
    if metric_value_kind == 'string':
        if metric_value_text is None:
            raise RuntimeError('String FRED payload requires metric_value_text')
        return (metric_value_text, None, None, None)
    raise RuntimeError(f'Unsupported FRED metric_value_kind: {metric_value_kind}')
...
class FREDNativeProjectorSummary:
...
def _aggregate_summaries(
    summaries: list[FREDNativeProjectorSummary],
) -> FREDNativeProjectorSummary:
    return FREDNativeProjectorSummary(
...
def project_fred_series_metrics_native(
    *,
    client: ClickhouseClient,
    database: str,
