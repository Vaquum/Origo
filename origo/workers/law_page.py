...
async function loadHistory(id,more=false){const key=historyKey(id);if(requests.has(key)||histories.has(key)&&!more)return;requests.add(key);const {start,end}=interval(),old=getHistory(key),q=new URLSearchParams({gate_id:id,from:start,to:end});if(more&&old?.next_cursor)q.set('cursor',old.next_cursor);try{const response=await fetch('/law/history.json?'+q);if(!response.ok)throw Error('History unavailable');const value=await response.json();value.events=[...(more?old.events:[]),...value.events];value.definitions_limited=Boolean(value.definitions_limited||(more&&old.definitions_limited));value.limited=Boolean(value.limited||(more&&old.limited));if(value.loading)value.error='History is still indexing; these observations are incomplete.';if(value.events.length>5000){value.events=value.events.slice(0,5000);value.limited=true;value.next_cursor=null}value.definitions=mergeDefinitions(value.events,[...(more?old.definitions||[]:[]),...(value.definitions||[])]);if(value.limited)value.error=value.definitions_limited?'Historical definitions are incomplete: lookup budget reached. Narrow the selected interval.':'History coverage is incomplete: a record or cache limit prevented a complete replay.';storeHistory(key,value);renderDetail();if(state.view==='recovery')renderContent()}catch{storeHistory(key,{error:'History unavailable. The current observation remains visible.',events:[]});renderDetail()}finally{requests.delete(key)}}
