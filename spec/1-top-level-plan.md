...
8a. FRED full-history backfill must also use a repo-native Dagster backfill path:
   ...
   6. live FRED revision-history replay must also respect the empirically proved response-size ceiling under the runtime timeout contract; the initial `vintage_dates` window must come from explicit env-backed runtime config, and with `ORIGO_FRED_HTTP_TIMEOUT_SECONDS=20` the current proved value is `ORIGO_FRED_REVISION_HISTORY_INITIAL_VINTAGE_DATES_PER_REQUEST=275`; any remaining `series/observations` edge `400 Bad Request` or timeout must trigger deterministic window splitting rather than a hard failure or snapshot fallback
