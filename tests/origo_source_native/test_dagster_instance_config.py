...
def test_max_runtime_covers_daily_asset_retry_envelope() -> None:
    ...
    The bundle maps each source's declared retry envelope 1:1 onto its
    canonical op, so the declared envelopes are the proof.
    """
    config = _validated_instance_config()
    ...
    for spec in SOURCE_REGISTRY:
        attempts = spec.orchestration.retry_count + 1
        required = int(
            spec.orchestration.retry_count * spec.orchestration.retry_delay
            + attempts * PER_ATTEMPT_EXECUTION_BUDGET_SECONDS
        )
        assert max_runtime >= required, spec.key
