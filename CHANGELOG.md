...
- A backfill never holds another source's fills back. Each native backfill, or bulk job run outside one, is one share of the ten-slot historical lane. Runs queue by start-time fair queuing: a run's place is the later of the service frontier (the highest place ever launched, recorded by the launcher) and one past its share's newest queued place. Concurrent backfills alternate, a late backfill starts at served service instead of jumping ahead of an older backlog, and deployment recovery keeps the places admission gave, continuing after them when restarted.
