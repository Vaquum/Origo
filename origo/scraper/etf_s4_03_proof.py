...
def run_s4_03_capability_proof() -> dict[str, Any]:
    started_at_utc = datetime.now(UTC)
    run_context = ScrapeRunContext(
        run_id=f's4-03-proof-{started_at_utc.strftime("%Y%m%dT%H%M%SZ")}',
    ...
    return {
        ...
        'proof_scope': 'Slice 4 S4-03 issuer adapters 1-10 capability proof',
        'run_id': run_context.run_id,
        'adapter_results': adapter_results,
    }


def write_s4_03_acceptance_proof(*, output_path: Path) -> dict[str, Any]:
    proof = run_s4_03_capability_proof()
...
if __name__ == '__main__':
    default_path = Path('spec/slices/slice-4-etf-use-case/acceptance-proof-s4-03.json')
    payload = write_s4_03_acceptance_proof(output_path=default_path)
