...
from origo.sources.adapters import binance_perp_agg_columnar as columnar
from origo.sources.adapters import binance_perp_agg_daily as daily
from origo.sources.adapters import binance_perp_agg_rest as rest
from origo.sources.adapters.binance_daily import Response

from .acceptance_cases import PERP_AGG_CASE, assert_archive_rest_equal
...
def test_real_perp_agg_archive_obeys_binance_authority(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(daily, 'get_response', archive_response)
    adapter = daily.BinancePerpAggDaily()
    partition = adapter.partition('2019-12-31')
    revision = adapter.fetch(partition)
    assert revision.row_count == 107365
    assert revision.key == 'ef24cf54b086fee7ce0e7050db3c91a2bc6d9b18778d5a4bd9668648ac94e5ee'
    assert next(iter(revision.rows()))[0] == 18266802
    adapter.revalidate(partition, revision)
    rows = tuple(daily.agg_csv_rows(_day_csv('2019-12-31'), partition))
    table = columnar.agg_table(_day_csv('2019-12-31'), partition)
    assert len(rows) == table.num_rows == 107365
    assert rows[0][0] == rows[-1][0] - 107364 == 18266802
    assert table.column('agg_trade_id').to_pylist() == [row[0] for row in rows]
    provenance = json.loads(
        (ARCHIVES / 'BTCUSDT-aggTrades-2019-12-31.provenance.json').read_text()
    )
    assert provenance['official_row_count'] == 107365
...
def test_perp_agg_archives_are_seven_column_rows_with_era_header() -> None:
    adapter = daily.BinancePerpAggDaily()
    assert daily.BinancePerpAggDaily.HEADER == tuple(columnar.HEADER.split(','))
    assert columnar.HEADER.startswith('agg_trade_id,')
    headed = _day_csv('2024-04-20')
    assert headed.splitlines()[0].decode() == columnar.HEADER
    assert headed.splitlines()[1].split(b',')[0] == b'2146019888'
    partition = adapter.partition('2024-04-20')
    prefix = b'\n'.join(headed.splitlines()[:5002]) + b'\n'
    rows = tuple(daily.agg_csv_rows(prefix, partition))
    assert len(rows) == 5001
    assert rows[0][0] == 2146019888
    table = columnar.agg_table(headed, partition)
    assert table.num_rows == 1257177
    assert table.column('agg_trade_id').to_pylist()[0] == 2146019888
    legacy = _day_csv('2019-12-31')
    assert legacy.splitlines()[0].split(b',')[0] == b'18266802'
    legacy_rows = tuple(daily.agg_csv_rows(legacy, adapter.partition('2019-12-31')))
    assert len(legacy_rows) == 107365
    with pytest.raises(ValueError, match='exactly seven fields'):
        tuple(daily.agg_csv_rows(prefix + b'1,2,3,4,5,6,true,extra\n', partition))
    with pytest.raises(ValueError, match='unsigned integers'):
        tuple(
            daily.agg_csv_rows(
                b'-1,2,3,4,5,1713571200005,true\n', partition
            )
        )


def test_perp_agg_field_parsers_reject_bad_input() -> None:
    adapter = daily.BinancePerpAggDaily()
    partition = adapter.partition('2019-12-31')
    ...
    with pytest.raises(ValueError, match='outside its partition'):
        tuple(
            daily.agg_csv_rows(
                body + b'9999999999,10000.0,0.001,9999999998,9999999999,1577836800000,true\n',
                partition,
            )
        )
    with pytest.raises(ValueError, match='precedes the declared first day'):
        adapter.partition('2019-12-30')
    assert adapter.candidate(datetime(2020, 1, 2, 12, tzinfo=UTC)).key == '2020-01-01'
...
def test_real_perp_agg_closed_minutes_obey_binance_provisional_rules(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv('BINANCE_API_KEY', raising=False)
    provenance, bodies = _rest_responses('provenance.json')
    assert provenance['minute_start'] == '2026-09-17T14:28:00+00:00'
    ...
    adapter = rest.BinancePerpAggProvisional()
    now = datetime(2026, 9, 17, 16, 0, tzinfo=UTC)
    covered = (
        adapter.partition('2026-09-17T14:29:00Z'),
        adapter.partition('2026-09-17T14:30:00Z'),
    )
    current, *missing = adapter.candidates(now, datetime(2026, 9, 17, 14, 28, tzinfo=UTC), covered)
    assert current.key == '2026-09-17T15:59:00Z'
    assert [part.key for part in missing] == [
        '2026-09-17T14:28:00Z',
        '2026-09-17T14:31:00Z',
        '2026-09-17T14:32:00Z',
        '2026-09-17T14:33:00Z',
        '2026-09-17T14:34:00Z',
    ]
    minute = adapter.partition('2026-09-17T14:28:00Z')
    ...
    archive_rows = tuple(
        daily.agg_csv_rows(
            (ARCHIVES / 'minute-2026-09-17T14-28.csv').read_bytes(),
            daily.BinancePerpAggDaily().partition('2026-09-17'),
        )
    )
    assert len(rows) == 5219
    assert_archive_rest_equal(archive_rows, rows, case=PERP_AGG_CASE)


def test_perp_agg_empty_minute_requires_two_ticks_and_later_boundary_evidence(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv('BINANCE_API_KEY', raising=False)
    ...
    adapter = rest.BinancePerpAggProvisional()
    partition = adapter.partition('2026-09-19T06:00:00Z')
...
_QUIRK_FIRST = b'2980094258,64000.10,0.00100,5000000000,5000000000,1713571200005,true\n'
_QUIRK_SENTINEL = b'2980094259,0,0,-1,-1,1713571199005,false\n'
_QUIRK_SECOND = b'2980094260,64001.00,0.00200,5000000001,5000000001,1713571201005,true\n'


def test_perp_agg_cleaning_drops_sentinels_and_exact_duplicates() -> None:
    adapter = daily.BinancePerpAggDaily()
    partition = adapter.partition('2024-04-20')
...
def test_perp_agg_cleaning_rejects_reused_ids_with_different_rows() -> None:
    adapter = daily.BinancePerpAggDaily()
    partition = adapter.partition('2024-04-20')
...
def test_perp_agg_cleaning_passes_clean_archives_through_untouched() -> None:
    adapter = daily.BinancePerpAggDaily()
    partition = adapter.partition('2019-12-31')
    body = _day_csv('2019-12-31')
    cleaned, dropped = adapter.clean_rows(body, partition)
...
def test_perp_agg_fetch_records_dropped_rows_in_evidence(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    adapter = daily.BinancePerpAggDaily()
    partition = adapter.partition('2024-04-20')
