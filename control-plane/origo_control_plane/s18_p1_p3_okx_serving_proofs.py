...
_PROOF_DB_SUFFIX_RUN_1 = '_s18_p1_p3_proof_run_1'
_PROOF_DB_SUFFIX_RUN_2 = '_s18_p1_p3_proof_run_2'
...
def _build_fixture_csv_bytes() -> bytes:
    return (
        ...
        b'BTC-USDT,3001,buy,42500.10000000,0.01000000,1704326400100\n'
        b'BTC-USDT,3002,sell,42500.20000000,0.02000000,1704326400700\n'
        b'BTC-USDT,3003,buy,42499.90000000,0.01500000,1704326401100\n'
        b'BTC-USDT,3004,sell,42500.40000000,0.03000000,1704326401900\n'
        b'BTC-USDT,3005,buy,42500.30000000,0.02500000,1704326402100\n'
    )
...
def _native_fingerprint(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    size_sum = sum(
        Decimal(str(cast(float, row['size'])))
        for row in rows
    )
...
def _to_legacy_row(event: OKXSpotTradeEvent) -> tuple[object, ...]:
    price = float(Decimal(event.price_text))
    size = float(Decimal(event.size_text))
...
def _seed_legacy_okx_table(
    *,
    ...
    client.execute(
        f'''
        INSERT INTO {database}.okx_spot_trades
        (
            instrument_name,
            trade_id,
            side,
...
def _run_single_database(*, settings: MigrationSettings, proof_database: str) -> dict[str, Any]:
    ...
    try:
        ...
        events = parse_okx_spot_trade_csv(fixture_csv)

        _seed_legacy_okx_table(client=admin_client, database=proof_database, events=events)

        write_summary = write_okx_spot_trades_to_canonical(
            client=admin_client,
            database=proof_database,
            events=events,
            run_id='s18-p1-p3-proof-write',
            ingested_at_utc=_INGESTED_AT_UTC,
        )
        if write_summary['rows_processed'] != len(events):
            raise RuntimeError('S18-P1 expected all fixture events to be processed by writer')
        ...
        native_projection_summary = project_okx_spot_trades_native(
            ...
            run_id='s18-p1-p3-proof-native-projector',
        ...
        aligned_projection_summary = project_okx_spot_trades_aligned(
            ...
            database=proof_database,
            partition_ids=partition_ids,
            run_id='s18-p1-p3-proof-aligned-projector',
        ...
        if not native_parity or not aligned_parity:
            ...
            raise RuntimeError(
                'S18-P2 parity mismatch against legacy OKX baselines: '
...
def run_s18_p1_p3_proof() -> dict[str, Any]:
    ...
    if not deterministic_match:
        raise RuntimeError('S18-P3 determinism failure: run fingerprints differ')

    acceptance_payload = {
        'proof_scope': (
            'Slice 18 S18-P1 fixed-window acceptance for OKX native and aligned_1s '
    ...
    parity_payload = {
        'proof_scope': (
            'Slice 18 S18-P2 parity checks versus legacy OKX fixture baselines '
    ...
    determinism_payload = {
        'proof_scope': (
            'Slice 18 S18-P3 replay determinism for OKX native and aligned_1s '
    ...
    return {
        'proof_s18_p1': acceptance_payload,
        'proof_s18_p2': parity_payload,
        'proof_s18_p3': determinism_payload,
        'baseline_fixture': baseline_fixture,
    }


def main() -> None:
    payload = run_s18_p1_p3_proof()
    _SLICE_DIR.mkdir(parents=True, exist_ok=True)

    p1_path = _SLICE_DIR / 'proof-s18-p1-acceptance.json'
    p2_path = _SLICE_DIR / 'proof-s18-p2-parity.json'
    p3_path = _SLICE_DIR / 'proof-s18-p3-determinism.json'
    baseline_path = _SLICE_DIR / 'baseline-fixture-2024-01-04_2024-01-04.json'

    p1_path.write_text(
        json.dumps(payload['proof_s18_p1'], indent=2, sort_keys=True) + '\n',
        encoding='utf-8',
    )
    p2_path.write_text(
        json.dumps(payload['proof_s18_p2'], indent=2, sort_keys=True) + '\n',
        encoding='utf-8',
    )
    p3_path.write_text(
        json.dumps(payload['proof_s18_p3'], indent=2, sort_keys=True) + '\n',
        encoding='utf-8',
    )
