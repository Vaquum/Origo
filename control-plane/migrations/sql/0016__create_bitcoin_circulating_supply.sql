CREATE TABLE IF NOT EXISTS {{DATABASE}}.bitcoin_circulating_supply (
    block_height UInt32 CODEC(Delta(4), ZSTD(3)),
    block_hash String CODEC(ZSTD(3)),
    block_timestamp UInt64 CODEC(Delta, ZSTD(3)),
    circulating_supply_sats UInt64 CODEC(Delta(8), ZSTD(3)),
    circulating_supply_btc Float64 CODEC(Delta, ZSTD(3)),
