...
def _operations_totals(records: dict[str, bytes], *, loading: bool, limited: bool) -> Document:
    ...
    for index, (metric, _) in enumerate(OPERATIONS):
        ...
        for horizon, minutes in (('60m', 60), ('24h', 1440)):
            ...
            complete = bool(end and not loading and not capped and not overlaps and not crossing
            ...
            reason = ('complete' if complete else 'history_loading' if loading else 'history_limited' if limited
                      ...
                      else 'counts_limited' if capped else 'missing_intervals' if accepted else 'history_unavailable')
            horizons[horizon] = {
...
class TapeCache:
    ...
    def _remember_operations(self, record: Document) -> None:
        ...
        if prior is not None and prior != packed:
    ...
    def _refresh_operations(self) -> None:
        self.operations_history = _operations_totals(self.operations_samples, loading=self.loading, limited=self.history_limited)
...
const LAW_NAMES={R1:'Live trades are fresh',C1:'Yesterday\u2019s archive is ready',C2:'Historical coverage is complete',D1:'Depth minutes are complete'};
...
const state=Object.fromEntries(new URLSearchParams(location.search));state.view=state.view==='gates'?'laws':['overview','sources','laws','recovery'].includes(state.view)?state.view:'overview';
...
const family=id=>/^law\.(R1|C1|C2|D1):/.exec(id||'')?.[1],lawName=g=>LAW_NAMES[family(g?.id)]||g?.name||'Unrecorded law';
...
function select(values){if(!detailOrigin&&(values.overview||values.gate||values.projection||values.source))detailOrigin={state:{...state},scroll:scrollY};history.replaceState({scroll:scrollY},'',location.href);Object.assign(state,values);for(const k of Object.keys(state))if(!state[k])delete state[k];cancelHistory();history.pushState({scroll:scrollY},'',location.pathname+'?'+new URLSearchParams(state));render()}
function closeDetail(){const origin=detailOrigin;detailOrigin=null;if(origin){for(const key of Object.keys(state))delete state[key];Object.assign(state,origin.state);cancelHistory();history.pushState({scroll:origin.scroll},'',location.pathname+'?'+new URLSearchParams(state));render();requestAnimationFrame(()=>scrollTo(0,origin.scroll))}else select({overview:null,projection:null,gate:null,source:null})}
...
window.addEventListener('resize',()=>requestAnimationFrame(drawWires));window.addEventListener('popstate',e=>{cancelHistory();for(const k of Object.keys(state))delete state[k];Object.assign(state,Object.fromEntries(new URLSearchParams(location.search)));state.view=state.view==='gates'?'laws':state.view||'overview';render();requestAnimationFrame(()=>scrollTo(0,e.state?.scroll||0))});
...
function trend(events,key='age_seconds',width=710){const unit=key.includes('seconds')?'s':'';const pts=events.map(e=>({x:Date.parse(e.evaluated_at),y:e.evidence?.[key]})).filter(p=>typeof p.y==='number'&&Number.isFinite(p.y)).sort((a,b)=>a.x-b.x);if(!pts.length)return '<div class="trend-empty">No numeric observations in this interval.</div>';const lo=pts[0].x,hi=pts.at(-1).x,max=Math.max(1,...pts.map(p=>p.y));let d='';pts.forEach((p,i)=>{const x=42+(p.x-lo)/Math.max(1,hi-lo)*(width-70),y=107-p.y/max*85;d+=(i&&p.x-pts[i-1].x<90000?'L':'M')+x+','+y+' ';if(pts.length===1)d+='l3,0 '});return `<svg class="spark" viewBox="0 0 ${width} 130" role="img" aria-label="Observed ${esc(label(key))}; gaps are not interpolated"><line x1="42" y1="107" x2="${width-28}" y2="107"/><text x="2" y="22">${Math.round(max)}${unit}</text><text x="12" y="110">0${unit}</text><path d="${d}"/><text x="42" y="128">${esc(new Date(lo).toISOString().slice(11,16))} UTC</text><text x="${width-95}" y="128">${esc(new Date(hi).toISOString().slice(11,16))} UTC</text></svg>`}
