...
OKXAlignedDataset = Literal['okx_spot_trades']


@dataclass(frozen=True)
class OKXAligned1sMaterialization:
    table_name: str
    id_column: str
    datetime_column: str
    price_column: str
...
_OKX_ALIGNED_1S_MATERIALIZATIONS: dict[
    OKXAlignedDataset, OKXAligned1sMaterialization
] = {
    'okx_spot_trades': OKXAligned1sMaterialization(
        table_name='okx_spot_trades',
        id_column='trade_id',
        datetime_column='datetime',
        price_column='price',
        quantity_column='size',
    )
}
...
def _build_grouped_query(
    *,
    database: str,
    materialization: OKXAligned1sMaterialization,
    where_clause: str,
) -> str:
...
def build_okx_aligned_1s_sql(
    *,
    dataset: OKXAlignedDataset,
    window: QueryWindow,
    database: str,
) -> str:
    ...
    materialization = _OKX_ALIGNED_1S_MATERIALIZATIONS[dataset]
    ...
    return ('SELECT * FROM (' f'{grouped_query}' f') ORDER BY {_ALIGNED_MS_ALIAS} ASC')
