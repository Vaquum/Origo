"""Benchmark real cached spot archives through the source build in an owned ClickHouse.

No production connection is used. Every scenario runs on the same container and
archives and includes the source build, its retained-content check and the declared
file outputs.
"""
...
def worker(args: tuple[str, dict[str, str], str, str]) -> dict[str, object]:
    day, env, archives, locks = args
    os.environ.update(env)
    ...
    try:
        ...
        spec = replace(
            ...
            components=tuple(
                replace(component, build=timed_component(component.key, component.build, timings))
        ...
        with archive_session():
            ...
            rows = runtime.store.execute(
                f"""SELECT row_count FROM {runtime.store.table('source_component_log')}
                WHERE source_key=%(source)s AND partition_key=%(partition)s
                  AND build_id=%(build)s AND component='raw'""",
                {'source': spec.key, 'partition': day, 'build': record.build_id},
            )
            result = {
                'day': day,
                'rows': int(str(rows[0][0])),
                'build_seconds': built - started,
                'integrity_seconds': time.perf_counter() - built,
                'ingest_seconds': timings['archive_decode_seconds'] + timings['raw'],
                'projection_seconds': sum(
                    timings[c.key] for c in spec.components if not c.provisional and c.key != 'raw'
                ),
...
def scenario(
    workers: int, days: list[str], env: dict[str, str], archives: Path, root: Path
) -> dict[str, object]:
    ...
    try:
        ...
        with ProcessPoolExecutor(max_workers=workers) as pool:
            measurements = list(
                pool.map(
                    worker,
                    [(day, settings, str(archives), str(runtime.lock_root)) for day in days],
                )
            )
...
def main() -> None:
    ...
    report: dict[str, object] = {
        ...
        'scope': 'Cached official archives; worker startup included, Dagster orchestration excluded. Each scenario runs the source build, its retained-content check and all declared shadow files. No Hugging Face network upload.',
    ...
    with (
        ...
        for workers in args.workers:
            result = scenario(workers, args.days, env, args.archives.resolve(), Path(directory))
