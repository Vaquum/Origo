...
def create_binance_agg_trades_table(context: AssetExecutionContext):
    ...
    try:
        ...
        table_exists = client.execute(
            f"SELECT count() FROM system.tables WHERE database = '{CLICKHOUSE_DATABASE}' AND name = 'binance_agg_trades'"
        )
        was_dropped = False
        ...
        if table_exists[0][0]:
            context.log.info(
                f'Table {CLICKHOUSE_DATABASE}.binance_agg_trades already exists. Dropping it...'
            )
            client.execute(
                f'DROP TABLE IF EXISTS {CLICKHOUSE_DATABASE}.binance_agg_trades'
            )
            context.log.info(
                f'Table {CLICKHOUSE_DATABASE}.binance_agg_trades has been dropped.'
            )
            was_dropped = True
        ...
        """)
        context.log.info(
            f'Table {CLICKHOUSE_DATABASE}.binance_agg_trades has been created successfully.'
        )
        ...
        context.log.error(f'Error creating binance_agg_trades table: {e!s}')
