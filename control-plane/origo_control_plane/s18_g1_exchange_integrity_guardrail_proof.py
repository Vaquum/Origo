...
_PROOF_DB_SUFFIX = '_s18_g1_proof'
...
def _set_proof_runtime_env(*, proof_database: str) -> dict[str, str | None]:
    quarantine_path = (
        Path.cwd() / 'storage' / 'audit' / f'{proof_database}-s18-g1-quarantine.json'
    )
    runtime_audit_path = (
        Path.cwd() / 'storage' / 'audit' / f'{proof_database}-s18-g1-runtime-audit.log'
    )
...
def _build_gap_csv_bytes() -> bytes:
    return (
        ...
        b'BTC-USDT,7001,buy,42900.10000000,0.01000000,1704326400100\n'
        b'BTC-USDT,7003,sell,42900.30000000,0.02000000,1704326400900\n'
    )


def _run_ingest_integrity_guardrail_proof(events: list[OKXSpotTradeEvent]) -> dict[str, Any]:
    ...
    if ingest_error is None:
        raise RuntimeError(
            'S18-G1 ingest integrity proof expected sequence-gap failure but got success'
        )
    if 'sequence-gap' not in ingest_error:
        raise RuntimeError(
            'S18-G1 ingest integrity proof expected sequence-gap error message, '
            f'got: {ingest_error}'
        )
...
def _run_projection_integrity_guardrail_proof(
    *,
    ...
    write_summary = write_okx_spot_trades_to_canonical(
        ...
        events=events,
        run_id='s18-g1-projection-write',
        ingested_at_utc=datetime(2026, 3, 10, 22, 40, 0, tzinfo=UTC),
    )
    if write_summary['rows_processed'] != len(events):
        raise RuntimeError('S18-G1 projection proof expected full canonical write')
    ...
    if projection_error is None:
        raise RuntimeError(
            'S18-G1 projection integrity proof expected sequence-gap failure but got success'
        )
    if 'sequence-gap' not in projection_error:
        raise RuntimeError(
            'S18-G1 projection integrity proof expected sequence-gap error message, '
            f'got: {projection_error}'
        )

    projected_rows_raw = admin_client.execute(
        f'SELECT count() FROM {proof_database}.canonical_okx_spot_trades_native_v1'
    )
    projected_rows = int(projected_rows_raw[0][0])
    if projected_rows != 0:
        raise RuntimeError(
            'S18-G1 projection integrity proof expected no projected rows on failure, '
    ...
    canonical_rows_raw = admin_client.execute(
        ...
        ''',
        {'source_id': 'okx', 'stream_id': 'okx_spot_trades'},
    )
    canonical_rows = int(canonical_rows_raw[0][0])
    if canonical_rows != len(events):
        raise RuntimeError(
            'S18-G1 projection integrity proof expected canonical rows to remain intact, '
...
def run_s18_g1_proof() -> dict[str, Any]:
    ...
    try:
        ...
        events = parse_okx_spot_trade_csv(_build_gap_csv_bytes())
        ...
        return {
            'proof_scope': (
                'Slice 18 S18-G1 exchange integrity suite enforcement in '
                'OKX ingest and projection paths'
            ),
            'proof_database': proof_database,
...
def main() -> None:
    payload = run_s18_g1_proof()
    _SLICE_DIR.mkdir(parents=True, exist_ok=True)
    output_path = _SLICE_DIR / 'proof-s18-g1-exchange-integrity.json'
