...
_PROJECTOR_ID = 'bybit_spot_trades_native_v1'
_TARGET_TABLE = 'canonical_bybit_spot_trades_native_v1'
_INSERT_COLUMNS = (
    'symbol, trade_id, trd_match_id, side, price, size, quote_quantity, timestamp, datetime, '
    'tick_direction, gross_value, home_notional, foreign_notional, event_id, '
...
def _require_source_event_time(event: ProjectorEvent) -> datetime:
    if event.source_event_time_utc is None:
        raise RuntimeError(
            'Bybit native projector requires source_event_time_utc for every event'
        )
...
def _build_insert_row(event: ProjectorEvent) -> tuple[tuple[object, ...], tuple[object, ...]]:
    ...
    symbol = _require_str(payload, key='symbol')
    trade_id = _require_int(payload, key='trade_id')
    trd_match_id = _require_str(payload, key='trd_match_id')
    ...
    quote_quantity = _require_decimal_float(payload, key='quote_quantity')
    timestamp = _require_int(payload, key='timestamp')
    tick_direction = _require_str(payload, key='tick_direction')
    gross_value = _require_decimal_float(payload, key='gross_value')
    home_notional = _require_decimal_float(payload, key='home_notional')
    foreign_notional = _require_decimal_float(payload, key='foreign_notional')
...
class BybitNativeProjectorSummary:
...
def _aggregate_summaries(
    summaries: list[BybitNativeProjectorSummary],
) -> BybitNativeProjectorSummary:
    return BybitNativeProjectorSummary(
...
def project_bybit_spot_trades_native(
    *,
    client: ClickhouseClient,
    database: str,
