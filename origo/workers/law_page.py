...
const state=Object.fromEntries(new URLSearchParams(location.search));state.view=state.view==='gates'?'laws':['overview','sources','laws','recovery'].includes(state.view)?state.view:'overview';if(state.family)state.family=state.family.replace(/^law\.(R1|C1|C2|D1)$/,'$1');
