...
from origo.sources.adapters import binance_perp_daily
...
DAY = '2019-09-08'
ASSET = 'build_binance_perp_trades_canonical_revision_origo'
...
def ready_job(
    ...
    monkeypatch.setattr(binance_perp_daily, 'get_response', archive_response)
    ...
    FakeHfApi.calls = []
    monkeypatch.setattr(perp_consumers, 'HfApi', FakeHfApi)
    monkeypatch.setattr(
    ...
    spec = replace(
        ...
        orchestration=replace(BINANCE_PERP_TRADES_SPEC.orchestration, retry_count=0),
    )
...
def test_one_job_prepares_verifies_and_publishes_all_perp_files(
...
def test_perp_file_failure_fails_job_and_retry_keeps_verified_generation(
...
def test_perp_unavailable_day_requests_publication_and_preserves_completed_day(
    ready_job: tuple[SourceStore, DagsterInstance, SourceBundle], tmp_path: Path
) -> None:
    store, instance, bundle = ready_job
    job = Definitions(
    ...
    assert statuses['2019-09-10'].value == 'FAILED'
    for sensor in bundle.sensors:
        if any(
...
def test_perp_new_verified_data_automatically_requests_every_canonical_only_consumer(
    ready_job: tuple[SourceStore, DagsterInstance, SourceBundle], tmp_path: Path
) -> None:
    store, instance, bundle = ready_job
    ...
    assert canonical.execute_in_process(instance=instance, partition_key='2019-09-09').success
    assert store.canonical_ready()
    ...
    for consumer in canonical_only:
        ...
        with build_sensor_context(instance=instance, definitions=definitions) as context:
            assert sensor.evaluate_tick(context).run_requests == []
def test_perp_publication_follows_canonical_state_across_provisional_refreshes(
    ready_job: tuple[SourceStore, DagsterInstance, SourceBundle], tmp_path: Path
) -> None:
    ...
    def provisional_refresh() -> None:
        ...
        start = datetime(2019, 9, 9, tzinfo=UTC)
        partition = Partition(
    ...
    assert canonical_job.execute_in_process(instance=instance, partition_key='2019-09-09').success
    ...
    assert pinned['pinned_token'] == store.snapshot().token
def test_perp_canonical_day_retires_failed_provisional_intervals_inside_it(
    ready_job: tuple[SourceStore, DagsterInstance, SourceBundle], tmp_path: Path
) -> None:
    from uuid import uuid4
...
def test_perp_native_backfill_failure_releases_publication(tmp_path: Path, status: str) -> None:
    ...
    with DagsterInstance.local_temp(str(tmp_path)) as instance:
        for day, run_status in (
            ('2019-09-10', DagsterRunStatus.FAILURE),
            ('2019-09-09', DagsterRunStatus.SUCCESS),
        ):
        ...
        assert not backfill_active(instance, spec)
        assert not backfill_owns_publication(instance, spec)
def test_perp_native_job_backfill_waits_for_own_selected_generations(
    ready_job: tuple[SourceStore, DagsterInstance, SourceBundle], tmp_path: Path
) -> None:
    from dagster._core.execution.backfill import BulkActionStatus, PartitionBackfill
    ...
    assert job.execute_in_process(instance=instance, partition_key='2019-09-09').success
    ...
    parent = PartitionBackfill(
        ...
        partition_names=[DAY, '2019-09-09'],
    )
    ...
    assert instance.get_materialized_partitions(AssetKey(ASSET)) == {DAY, '2019-09-09'}
...
def test_perp_huggingface_upload_records_every_rendered_series(
    ready_job: tuple[SourceStore, DagsterInstance, SourceBundle],
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    from origo.sources.profiles import perp_consumers
    from origo.sources.profiles.formulas import perp_huggingface as perp_snapshot
    ...
    monkeypatch.setattr(perp_snapshot, 'get_perp_klines_from_1m_projection', lambda **kwargs: frame)
    monkeypatch.setattr(
        perp_snapshot, 'get_perp_dollar_klines', lambda **kwargs: frame.clear()
    )
    destination = str(tmp_path / 'files' / store.spec.key / 'huggingface')
    perp_consumers._huggingface(store, store.snapshot(), destination)
