...
def create_binance_trades_week_of_year_summary(context: AssetExecutionContext):
    ...
    try:
        ...
        table_exists = client.execute(
            f"SELECT count() FROM system.tables WHERE database = '{CLICKHOUSE_DATABASE}' AND name = 'binance_trades_week_of_year_summary'"
        )
        was_dropped = False
        ...
        if table_exists[0][0]:
            context.log.info(
                f'Table {CLICKHOUSE_DATABASE}.binance_trades_week_of_year_summary already exists. Dropping it...'
            )
            client.execute(
                f'DROP TABLE IF EXISTS {CLICKHOUSE_DATABASE}.binance_trades_week_of_year_summary'
            )
            context.log.info(
                f'Table {CLICKHOUSE_DATABASE}.binance_trades_week_of_year_summary has been dropped.'
            )
            was_dropped = True
        else:
            context.log.info(
                f'Table {CLICKHOUSE_DATABASE}.binance_trades_week_of_year_summary does not exist. Creating it...'
            )

        # Create the binance_trades_week_of_year_summary table
        context.log.info(
            f'Creating table {CLICKHOUSE_DATABASE}.binance_trades_week_of_year_summary...'
        )
        client.execute(f"""
        ...
        """)
        context.log.info(
            f'Table {CLICKHOUSE_DATABASE}.binance_trades_week_of_year_summary has been created successfully.'
        )

        # Backfill the table with aggregated data
        context.log.info(
            f'Backfilling {CLICKHOUSE_DATABASE}.binance_trades_week_of_year_summary with aggregated data...'
        )
        client.execute(f"""
        ...
        """)
        context.log.info(
            f'Successfully backfilled table {CLICKHOUSE_DATABASE}.binance_trades_week_of_year_summary'
        )

        # Verify the data was inserted
        count_result = client.execute(
            f'SELECT count() FROM {CLICKHOUSE_DATABASE}.binance_trades_week_of_year_summary'
        )
        row_count = count_result[0][0]
        context.log.info(
            f'Inserted {row_count} week-of-year summaries into {CLICKHOUSE_DATABASE}.binance_trades_week_of_year_summary'
        )

        # Get min and max week for verification
        min_week_result = client.execute(
            f'SELECT min(week_of_year) FROM {CLICKHOUSE_DATABASE}.binance_trades_week_of_year_summary'
        )
        max_week_result = client.execute(
            f'SELECT max(week_of_year) FROM {CLICKHOUSE_DATABASE}.binance_trades_week_of_year_summary'
        )
        ...
        context.log.error(
            f'Error creating binance_trades_week_of_year_summary table: {e!s}'
        )
