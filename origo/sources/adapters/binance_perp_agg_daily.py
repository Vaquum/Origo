...
from .binance_perp_agg_columnar import COLUMNS as AGG_COLUMNS
from .binance_perp_agg_columnar import HEADER as AGG_HEADER
from .binance_perp_agg_columnar import agg_table


def parse_decimal(text: str) -> Decimal:
    return shared_parse_decimal(text, noun='Perp aggregate price and quantity')
...
class BinancePerpAggDaily(BinanceArchiveDaily):
    first_day: date = date(2019, 12, 31)
    SOURCE_KEY: ClassVar[str] = 'binance_perp_aggtrades'
    ARCHIVE_BASE_URL_ENV: ClassVar[str] = 'BINANCE_PERP_AGGTRADES_ARCHIVE_BASE_URL'
    ...
    HEADER: ClassVar[tuple[str, ...] | None] = tuple(AGG_HEADER.split(','))
...
def agg_csv_rows(body: bytes, partition: Partition) -> Iterator[Row]:
    return BinancePerpAggDaily().parse_rows(body, partition)
