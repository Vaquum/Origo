...
def _legacy_row(
    feed: str, series: str, status: str, recorded_at: datetime, *, minute: datetime = MINUTE,
    sha256: str = '', rows: int = 0, error_code: str = '',
) -> tuple[object, ...]:
    return (
        feed, series, minute.replace(tzinfo=None), rows, sha256, 0, status, error_code, '',
        'legacy-host', recorded_at,
    )
...
def _started_owner_child(environment: dict[str, str], root: str, sender: Connection) -> None:
    ...
    record_receipt(
        client, ORIGO_DATABASE, feed='provisional', series='perp:mount', minute=MINUTE,
        rows=0, sha256=TOKEN, duration_ms=0, status='STARTED', attempt=attempt,
    )
...
def test_dead_attempt_preserves_token_and_fences_its_owner(
    ...
    try:
        ...
        identity = AttemptIdentity('perp:mount:' + TOKEN, UUID(attempt_text), epoch, TOKEN, 'perp:mount')
        live = live_owner.attempt('perp:mount:new', state_token='b' * 64, prerequisite_key='perp:mount')
        record_receipt(
            client, ORIGO_DATABASE, feed='provisional', series='perp:mount', minute=MINUTE,
            rows=0, sha256='b' * 64, duration_ms=0, status='STARTED', attempt=live,
        )
        ...
        assert reconcile_died_receipts(
            client, ORIGO_DATABASE, feed='provisional', now=later,
        ...
        assert failed_attempts(client, ORIGO_DATABASE, feed='provisional', series='perp:mount', token=TOKEN)[0] == 1
        assert failed_attempts(client, ORIGO_DATABASE, feed='provisional', series='perp:mount', token='b' * 64)[0] == 0
        ...
        assert terminal == [
            ('FAILED', 'WORKER_DIED', TOKEN, TOKEN, identity.work_id, identity.attempt_id, epoch, 'perp:mount')
        ]
        ...
        with pytest.raises(SourceError, match='retired'):
            record_receipt(
                client, ORIGO_DATABASE, feed='provisional', series='perp:mount', minute=MINUTE,
                rows=1, sha256=TOKEN, duration_ms=1, status='OK', attempt=identity,
            )
        with pytest.raises(SourceError, match='retired'):
            record_receipt(
                client, ORIGO_DATABASE, feed='provisional', series='perp:mount', minute=MINUTE,
                rows=0, sha256=TOKEN, duration_ms=0, status='STARTED', attempt=identity,
            )
        assert client.execute(f'SELECT count() FROM {TABLE} WHERE attempt_id=%(a)s', {'a': identity.attempt_id}) == [(2,)]
        assert outstanding_owner_epochs(client, ORIGO_DATABASE, 'provisional') == (live_owner.epoch,)

        # The live attempt on the same minute was untouched and completes normally, once.
        record_receipt(
            client, ORIGO_DATABASE, feed='provisional', series='perp:mount', minute=MINUTE,
            rows=3, sha256='b' * 64, duration_ms=5, status='OK', attempt=live,
        )
        with pytest.raises(SourceError, match='already has a terminal'):
            record_receipt(
                client, ORIGO_DATABASE, feed='provisional', series='perp:mount', minute=MINUTE,
                rows=0, sha256='b' * 64, duration_ms=5, status='FAILED', attempt=live,
            )
...
def test_receipt_schema_keeps_old_writers_through_upgrade_merges_and_rollback(
    ...
    try:
        ...
        _old_writer(
            client,
            _legacy_row('legacy', 'other', 'OK', MINUTE + timedelta(seconds=1), sha256='legacy', rows=1),
            _legacy_row('legacy', 'other', 'FAILED', MINUTE + timedelta(seconds=2), error_code='LEGACY'),
        )
        client.execute(
        ...
        for status, rows in (('STARTED', 0), ('OK', 7)):
            record_receipt(
                client, ORIGO_DATABASE, feed='legacy', series='perp:mount', minute=MINUTE,
                rows=rows, sha256=TOKEN, duration_ms=1, status=status, attempt=attempt,
            )
        failing = owner.attempt('other:' + str(MINUTE))
        record_receipt(
            client, ORIGO_DATABASE, feed='legacy', series='other', minute=MINUTE, rows=0,
            sha256='', duration_ms=1, status='FAILED', error_code='PROBE', attempt=failing,
        )

        def snapshot() -> list[tuple[object, ...]]:
            return client.execute(f'SELECT *, {IDENTITY} FROM {TABLE} ORDER BY feed, series, recorded_at')
        ...
        assert all(row[11:16] == (attempt.work_id, attempt.attempt_id, owner.epoch, TOKEN, 'perp:mount') for row in modern)
        ...
        assert client.execute(f'SELECT count() FROM system.parts WHERE database=%(d)s AND table=%(t)s AND active', {'d': ORIGO_DATABASE, 't': 'worker_minute_log'}) == [(1,)]
        ...
        _old_writer(client, _legacy_row('legacy', 'rollback', 'OK', MINUTE + timedelta(seconds=3), rows=2))
        ensure_monitoring_tables(client, ORIGO_DATABASE)
        after = snapshot()
        ...
        failed = failed_receipts_since(
            client, ORIGO_DATABASE, MINUTE - timedelta(days=1), datetime.now(UTC) + timedelta(minutes=1)
        )
        assert [(row.feed, row.series, row.error_code) for row in failed] == [
            ('legacy', 'other', 'LEGACY'), ('legacy', 'other', 'PROBE'),
        ]
        assert failed_attempts(client, ORIGO_DATABASE, feed='legacy', series='other', minute=MINUTE) == (
            2, failed[1].recorded_at,
        )
        ...
        with caplog.at_level(logging.ERROR, logger='origo.workers.receipts'):
            assert reconcile_died_receipts(client, ORIGO_DATABASE, feed='legacy', now=MINUTE + timedelta(days=1)) == 0
        assert ['1 unpaired legacy receipts retain UNKNOWN ownership' in record.message for record in caplog.records] == [True]
        assert legacy_unresolved_page(client, ORIGO_DATABASE, 'legacy', MINUTE + timedelta(days=1)).rows == (
            receipt_identity.LegacyReceipt('subset', MINUTE, '', MINUTE),
        )
        assert client.execute(f"SELECT count() FROM {TABLE} WHERE error_code='WORKER_DIED'") == [(0,)]
        ...
        client.execute(
            LEGACY_DDL.replace("recorded_at DateTime64(3, 'UTC'))", "recorded_at DateTime64(3, 'UTC'), "
                               'work_id String, attempt_id UUID, owner_epoch String, state_token String, '
                               'prerequisite_key String, event_id UUID)')
            .replace('ORDER BY (feed,series,minute,recorded_at)', 'ORDER BY (feed,series,minute,recorded_at,event_id)')
        )
        with pytest.raises(SourceError, match='identity schema is inconsistent'):
            ensure_monitoring_tables(client, ORIGO_DATABASE)
    finally:
...
def test_same_minute_receipts_never_collapse_and_conflicting_retries_fail(
    ...
    def depth(
        status: Literal['OK', 'FAILED', 'STARTED'], *, attempt: AttemptIdentity | None = None,
        rows: int = 0, sha256: str = '', duration_ms: int = 0, error_code: str = '',
    ) -> None:
        record_receipt(
            client, ORIGO_DATABASE, feed='depth', series='depth20_snapshots', minute=MINUTE,
            rows=rows, sha256=sha256, duration_ms=duration_ms, status=status,
    ...
    try:
        ...
        stamps = [row[0] for row in client.execute(f'SELECT recorded_at FROM {TABLE} ORDER BY recorded_at')]
        ...
        publish = first.attempt('perp:mount:' + TOKEN, state_token=TOKEN, prerequisite_key='perp:mount')
        with pytest.raises(ValueError, match='intended state token'):
            record_receipt(
                client, ORIGO_DATABASE, feed='depth', series='perp:mount', minute=MINUTE,
                rows=0, sha256='x', duration_ms=0, status='STARTED', attempt=publish,
            )
        assert client.execute(f'SELECT count() FROM {TABLE}') == [(7,)]
    finally:
        first.close()
...
def test_receipt_clock_collisions_are_bounded(
    ...
    def probe(rows: int) -> None:
        record_receipt(
            client, ORIGO_DATABASE, feed='probe', series='unit', minute=MINUTE,
            rows=rows, sha256='', duration_ms=0, status='OK',
        )
    ...
    try:
        ...
        assert client.execute(f'SELECT recorded_at FROM {TABLE} ORDER BY recorded_at') == [
            (stored,), (stored + timedelta(milliseconds=5),),
        ]
    finally:
        client.disconnect()
...
def test_legacy_unresolved_receipts_are_paged_and_never_failed(
    ...
    try:
        ...
        _old_writer(client, *(_legacy_row('probe', 'unit', 'STARTED', old, minute=minute) for minute in minutes))
        _old_writer(client, *(_legacy_row('probe', 'unit', 'OK', old, minute=minute, rows=1) for minute in minutes[:100]))
        _old_writer(client, _legacy_row('probe', 'unit', 'STARTED', now, minute=MINUTE - timedelta(minutes=1)))
        record_receipt(
            client, ORIGO_DATABASE, feed='probe', series='unit', minute=MINUTE - timedelta(minutes=2),
            rows=0, sha256='', duration_ms=0, status='STARTED', attempt=owner.attempt('unit:modern'),
        )
        ...
        while True:
            ...
            assert all(row.series == 'unit' and row.recorded_at == old.replace(microsecond=old.microsecond // 1000 * 1000) for row in page.rows)
            cursor = page.next
            if cursor is None:
                break
        assert pages == 3 and seen == minutes[100:]
        assert legacy_unresolved_count(client, ORIGO_DATABASE, 'probe', cutoff, page_bound=2) == receipt_identity.LegacyUnresolved(2000, False)
        assert legacy_unresolved_count(client, ORIGO_DATABASE, 'probe', cutoff) == receipt_identity.LegacyUnresolved(2200, True)
        with caplog.at_level(logging.ERROR, logger='origo.workers.receipts'):
