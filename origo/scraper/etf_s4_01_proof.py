...
from .etf_adapters import build_s4_01_issuer_adapters
...
def run_s4_01_capability_proof() -> dict[str, Any]:
    started_at_utc = datetime.now(UTC)
    run_context = ScrapeRunContext(
        run_id=f's4-01-proof-{started_at_utc.strftime("%Y%m%dT%H%M%SZ")}',
        started_at_utc=started_at_utc,
    )

    adapter_results: list[dict[str, Any]] = []
    for adapter in build_s4_01_issuer_adapters():
    ...
    return {
        ...
        'proof_scope': 'Slice 4 S4-01 issuer adapters 1-3 capability proof',
        'run_id': run_context.run_id,
        'adapter_results': adapter_results,
    }


def write_s4_01_acceptance_proof(*, output_path: Path) -> dict[str, Any]:
    proof = run_s4_01_capability_proof()
...
if __name__ == '__main__':
    default_path = Path('spec/slices/slice-4-etf-use-case/acceptance-proof-s4-01.json')
    payload = write_s4_01_acceptance_proof(output_path=default_path)
