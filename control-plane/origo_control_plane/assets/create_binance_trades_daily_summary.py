...
def create_binance_trades_daily_summary(context: AssetExecutionContext):
    ...
    try:
        ...
        table_exists = client.execute(
            f"SELECT count() FROM system.tables WHERE database = '{CLICKHOUSE_DATABASE}' AND name = 'binance_trades_daily_summary'"
        )
        was_dropped = False
        ...
        if table_exists[0][0]:
            context.log.info(
                f'Table {CLICKHOUSE_DATABASE}.binance_trades_daily_summary already exists. Dropping it...'
            )
            client.execute(
                f'DROP TABLE IF EXISTS {CLICKHOUSE_DATABASE}.binance_trades_daily_summary'
            )
            context.log.info(
                f'Table {CLICKHOUSE_DATABASE}.binance_trades_daily_summary has been dropped.'
            )
            was_dropped = True
        else:
            context.log.info(
                f'Table {CLICKHOUSE_DATABASE}.binance_trades_daily_summary does not exist. Creating it...'
            )

        # Create the binance_trades_daily_summary table
        context.log.info(
            f'Creating table {CLICKHOUSE_DATABASE}.binance_trades_daily_summary...'
        )
        client.execute(f"""
        ...
        """)
        context.log.info(
            f'Table {CLICKHOUSE_DATABASE}.binance_trades_daily_summary has been created successfully.'
        )

        # Backfill the table with aggregated data
        context.log.info(
            f'Backfilling {CLICKHOUSE_DATABASE}.binance_trades_daily_summary with aggregated data...'
        )
        client.execute(f"""
        ...
        """)
        context.log.info(
            f'Successfully backfilled table {CLICKHOUSE_DATABASE}.binance_trades_daily_summary'
        )

        # Verify the data was inserted
        count_result = client.execute(
            f'SELECT count() FROM {CLICKHOUSE_DATABASE}.binance_trades_daily_summary'
        )
        row_count = count_result[0][0]
        context.log.info(
            f'Inserted {row_count} daily summaries into {CLICKHOUSE_DATABASE}.binance_trades_daily_summary'
        )

        # Get summary statistics for verification
        min_day_result = client.execute(
            f'SELECT min(day_start) FROM {CLICKHOUSE_DATABASE}.binance_trades_daily_summary'
        )
        max_day_result = client.execute(
            f'SELECT max(day_start) FROM {CLICKHOUSE_DATABASE}.binance_trades_daily_summary'
        )
        ...
        context.log.error(f'Error creating binance_trades_daily_summary table: {e!s}')
