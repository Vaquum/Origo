...
from .s17_p6_raw_fidelity_precision_proof import run_s17_p6_proof

_SLICE_DIR = Path('spec/slices/slice-17-fred-event-sourcing-port')


def main() -> None:
    p6_payload = run_s17_p6_proof()
    payload = {
        'proof_scope': (
            'Slice 17 S17-G4 raw-fidelity and precision guardrail verification '
            'for FRED canonical ingest path'
        ),
    ...
    output_path = _SLICE_DIR / 'proof-s17-g4-raw-fidelity-precision.json'
